...
impl BaseClient {
    ...
    pub fn create_http_client(
        ...
        for ca_path in &config.tls.trusted_ca {
            ...
            } else if config.tls.verify_server_cert {
                return Err(ClientError::Tls(TlsError::certificate_file(
                    ca_path,
                    "Trusted CA certificate file not found (required because verify_server_cert is enabled)"
                        .to_string(),
                )));
            } else {
                debug!("Skipping missing CA certificate (verify_server_cert is disabled): {ca_path}");
            }
        }
        debug!(
