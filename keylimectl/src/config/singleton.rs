...
pub fn get_config() -> &'static Config {
    if !is_initialized() {
        #[rustfmt::skip]
        panic!( //#[allow_ci]
            "Config not initialized. Run `keylimectl configure` to create a \
             configuration file, or check that initialization completed \
             successfully before running commands."
        );
    }
    GLOBAL_CONFIG.get().expect("Config not initialized") //#[allow_ci]
}
