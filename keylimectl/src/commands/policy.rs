...
//! # Command Types
//!
//! - [`PolicyAction::Create`]: Create a new runtime policy
...
//! let create_action = PolicyAction::Create {
...
/// # Arguments
///
/// * `action` - The specific policy action to perform (Create, Show, Update, or Delete)
...
mod tests {
    ...
    mod action_variants {
        ...
        fn test_create_action() {
            let action = PolicyAction::Create {
    ...
    mod policy_validation {
        ...
        fn test_valid_allowlist_structure() {
            ...
            let allowlist = policy["allowlist"].as_array().unwrap();
    ...
    mod json_responses {
        ...
        fn test_success_response_structure() {
            let response = json!({
                "status": "success",
                "message": "Runtime policy 'test-policy' created successfully",
    ...
    mod file_operations {
        ...
        fn test_read_valid_policy_file() {
            ...
            let file_path = policy_file.path().to_str().unwrap();
        ...
        fn test_read_invalid_policy_file() {
            let invalid_file = create_invalid_policy_file()
                .expect("Failed to create invalid file");
            let file_path = invalid_file.path().to_str().unwrap();
