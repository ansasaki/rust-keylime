...
pub(super) async fn add_agent(
    ...
    let agent_port = match params.port {
        Some(p) => Some(p),
        None => match agent_data.get("port").and_then(|v| v.as_u64()) {
            Some(n) => Some(u16::try_from(n).map_err(|_| {
                CommandError::invalid_parameter(
                    "port",
                    format!(
                        "Agent port {n} from registrar is out of range (0-65535)"
                    ),
                )
            })?),
            None => None,
