...
pub async fn execute(
    ...
    // Check readability before opening — gives a sudo hint on EACCES.
    privilege::check_file_readable(
        std::path::Path::new(file),
        &format!("policy sign {file}"),
    )
    .map_err(|e| KeylimectlError::Command(e.into()))?;
...
fn sign_x509(
    ...
    let signer = if let Some(kf) = keyfile {
        let cert_path = cert_file.ok_or_else(|| {
            KeylimectlError::validation(
                "X.509 backend requires --cert-file when using --keyfile",
            )
        })?;
        output.info(format!(
            "Loading key from {kf} and certificate from {cert_path}"
        ));
        let s = X509Signer::from_files(kf, cert_path).map_err(|e| {
            KeylimectlError::validation(format!(
                "Failed to load key/cert: {e}"
            ))
        })?;
        // Optionally export the loaded certificate to a different path.
        if let Some(out_path) = cert_outfile {
            std::fs::write(out_path, s.certificate_pem()).map_err(|e| {
                KeylimectlError::validation(format!(
                    "Failed to export certificate to '{out_path}': {e}"
                ))
            })?;
            output.info(format!("Certificate exported to {out_path}"));
        }
        s
    } else {
        output.info(
        ...
        // Generate without writing cert internally; write via certificate_pem().
        let s = X509Signer::generate(None).map_err(|e| {
            KeylimectlError::validation(format!(
        ...
        std::fs::write(cert_path, s.certificate_pem()).map_err(|e| {
            KeylimectlError::validation(format!(
