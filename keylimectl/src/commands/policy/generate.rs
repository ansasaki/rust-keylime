...
pub(super) async fn generate_runtime(
    ...
    let mut detected_log_hash_alg: Option<String> = None;
    ...
    if let Some(ima_path) = ima_measurement_list {
        ...
        if path.exists() {
            ...
            // Detect the IMA template hash algorithm (log_hash_alg)
            // from the template hash field, which may differ from the
            // file digest algorithm
            if detected_log_hash_alg.is_none() {
                detected_log_hash_alg = ima_data.detected_log_hash_alg;
            }
            ...
            return Err(CommandError::invalid_parameter(
                "ima_measurement_list",
                format!("IMA measurement list file not found: {ima_path}"),
            ));
        }
    }
    ...
    // Set the IMA template hash algorithm (log_hash_alg).
    // This is the algorithm used by IMA to hash template data, determined
    // from the template hash field length in the IMA log. It may differ
    // from the file digest algorithm (e.g., sha1 template hashes with
    // sha256 file digests is common when ima_hash=sha1 is used).
    if let Some(alg) = &detected_log_hash_alg {
        policy.set_log_hash_alg(alg.clone());
    }

    // Add IMA signature verification keys
    for key_path in add_ima_signature_verification_key {
