...
fn parse_repomd_location(xml: &str, data_type: &str) -> Option<String> {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Empty(e))
                if in_target_data
                    && e.local_name().as_ref() == b"location" =>
            {
                for attr in e.attributes().flatten() {
            ...
            Ok(Event::End(e)) if e.local_name().as_ref() == b"data" => {
...
fn parse_filelists_ext(
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(e)) if e.local_name().as_ref() == b"file" => {
                for attr in e.attributes().flatten() {
            ...
            Ok(Event::End(e)) if e.local_name().as_ref() == b"file" => {
                current_hash = None;
            }
            Ok(Event::Eof) => break,
            Err(e) => {
...
fn parse_primary_rpm_urls(
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::End(e)) if e.local_name().as_ref() == b"package" => {
                in_package = false;
            }
            Ok(Event::Eof) => break,
            Err(e) => {
...
const MAX_REPO_DECOMPRESSED_SIZE: u64 = 100_000_000; // 100 MB

/// Detect compression format and decompress data with size limit.
fn decompress_data(
    data: &[u8],
    source: &str,
) -> Result<Vec<u8>, PolicyGenerationError> {
    if data.len() >= 2 && data[0] == 0x1f && data[1] == 0x8b {
        ...
        let mut limited = decoder.take(MAX_REPO_DECOMPRESSED_SIZE);
        let mut decompressed = Vec::new();
        let _ = limited.read_to_end(&mut decompressed).map_err(|e| {
            PolicyGenerationError::RpmParse {
